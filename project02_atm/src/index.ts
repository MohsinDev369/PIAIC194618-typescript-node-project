...
interface retrunTypes {
    userId: string, userPin: string
}

async function start() {
    // Prompt the user for login details
    const { userId, userPin }: retrunTypes = await inquirer.prompt(loginPrompt);
    ...
    if (userId && userPin) {
        ...
        async function  atm_Menue() {
            const { options }: { options: string } = await inquirer.prompt(choisePrompt);

            if (options === 'Check Balance') {
                console.log('You have ' + chalk.cyanBright.bgGreen(userAmount));
                atm_Menue()
            } else if (options === 'Send Money') {
                console.log('x amount '+chalk.cyanBright.bgRed('Money Sent'));
                atm_Menue()
            }
